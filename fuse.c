...
char* memcpu(char* source, char* buf, long contentSize, long bufSize)
{
    long size = contentSize + bufSize;   
...
{  
    Link node = seekNode(tree, path);
    tempContent = memcpu(node->content,buf,node->size,size);
